...
GRAY = (0.50, 0.50, 0.50)
GREY = (0.50, 0.50, 0.50)
INK_GRAY = (76 / 255, 74 / 255, 72 / 255)
INK_GREY = (76 / 255, 74 / 255, 72 / 255)
...
MATHTEXT_FONTSET = 'stix'     # 使用数学文本的字体设置
...
def set_font_size(font_size=15, title_size=None, sup_title_size=None, label_size=None,
                  tick_size=None, legend_size=None, cbar_label_size=None,
                  cbar_tick_size=None, label_pad=None, title_pad=None, tag_size=None):
    ...
    if cbar_label_size is None:
        cbar_label_size = tick_size
    if cbar_tick_size is None:
...
def set_line_style(line_width=3., marker_size=None, tick_major_width=None, tick_minor_width=None, tick_major_size=None, tick_minor_size=None, axes_linewidth=None, star_size=None, legend_frame_linewidth=None, errorbar_linewidth=None):
    ...
    if tick_major_width is None:
        tick_major_width = line_width * 2 / 3
    if tick_minor_width is None:
        tick_minor_width = line_width / 3
    if tick_major_size is None:
        tick_major_size = line_width * 8 / 3
    if tick_minor_size is None:
        tick_minor_size = line_width * 4 / 3
...
plt.rcParams['axes.labelpad'] = LABEL_PAD       # 坐标轴标签与轴的距离
plt.rcParams['axes.titlepad'] = TITLE_PAD       # 标题与轴的距离
...
plt.rcParams['mathtext.fontset'] = MATHTEXT_FONTSET     # 使用数学文本的字体设置
...
def pop_dict_get_dir_flat(dict_data, value_dir_key, both_dir_key, basedir):
    '''
    弹出一部分参数,并且返回路径名,与pop_dict_get_dir不同的是,这里不会嵌套路径,而是将所有参数都放在同一级路径下,用下划线连接
    例如:
    basedir = '/home/user/experiments'
    dict_data = {'lr': 0.001, 'batch_size': 32, 'model': 'resnet'}
    value_dir_key = ['lr', 'batch_size']
    both_dir_key = ['model']
    则返回的basedir为:
    '/home/user/experiments/lr_0.001_batch_size_32_model_resnet'
    '''
    local_dict_data = dict_data.copy()
    if value_dir_key is None:
        value_dir_key = []
    ...
    after_basedir_parts = []
    for key in value_dir_key:
        after_basedir_parts.append(str(dict_data[key]))
        local_dict_data.pop(key)
    for key in both_dir_key:
        after_basedir_parts.append(concat_str([key, str(dict_data[key])]))
        local_dict_data.pop(key)
    after_basedir = '_'.join(after_basedir_parts)
    new_dir = os.path.join(basedir, after_basedir)
    return local_dict_data, new_dir
...
def plt_violin(ax, data, positions=None, labels=None, body_colors=None, line_colors=None, line_width=None, vert=True, text_process=None, **kwargs):
    '''
...
def add_colorbar(ax, mappable=None, cmap=CMAP, ticks=None, tick_labels=None, discrete_label=None, display_edge_ticks=True, cbar_position=None, cbar_label=None, use_mask=False, mask_color=MASK_COLOR, mask_pos='start', mask_pad=0, mask_cbar_ratio=None, mask_tick='mask', mask_tick_loc=None, label_size=None, tick_size=None, adjust_tick_size=True, tick_proportion=TICK_PROPORTION, label_kwargs=None, norm_mode='linear', vmin=None, vmax=None, norm_kwargs=None, text_process=None, formatter=None, formatter_kwargs=None, round_digits=ROUND_DIGITS, round_format_type=ROUND_FORMAT, add_leq=False, add_geq=False, inset_mode='fig'):
    ...
    :param cmap: 颜色条的颜色映射,默认为CMAP.可以是离散的或连续的.
    ...
    # 在norm_mode 'boundary' 时,用户需要输入norm_kwargs
    if norm_mode == 'boundary':
        if 'boundaries' not in norm_kwargs:
            raise ValueError('当norm_mode为boundary时,需要在norm_kwargs中输入boundaries参数')
        if 'ncolors' not in norm_kwargs:
            raise ValueError('当norm_mode为boundary时,需要在norm_kwargs中输入ncolors参数')
...
def add_star(ax, x, y, label=None, marker=STAR, color=RED, markersize=None, linestyle='None', **kwargs):
    '''
    在指定位置添加五角星标记.
    :param ax: matplotlib的轴对象,用于绘制图形.
    :param x: 五角星的x坐标.
...
def add_star_heatmap(ax, i, j, label=None, marker=STAR, color=RED, markersize=None, linestyle='None', **kwargs):
    '''
    在热图的指定位置添加五角星标记.
...
# region 初级作图函数(添加辅助线)
def add_vline(ax, x, label=None, color=RED, linestyle=AUXILIARY_LINE_STYLE, linewidth=None, **kwargs):
    '''
    在指定位置添加垂直线.
    :param ax: matplotlib的轴对象,用于绘制图形.
    :param x: 垂直线的x坐标.
...
def add_hline(ax, y, label=None, color=RED, linestyle=AUXILIARY_LINE_STYLE, linewidth=None, **kwargs):
    '''
    在指定位置添加水平线.
    :param ax: matplotlib的轴对象,用于绘制图形.
    :param y: 水平线的y坐标.
...
def add_sline(ax, slope, intercept, reference_point='auto', label=None, color=RED, linestyle=AUXILIARY_LINE_STYLE, linewidth=None, **kwargs):
    '''
    按照斜率和截距添加直线.(贯穿整个ax)
...
def add_grid(ax, x_list=None, y_list=None, color=RANA, linestyle=AUXILIARY_LINE_STYLE, linewidth=None, **kwargs):
    '''
    在指定位置添加网格线.
    :param ax: matplotlib的轴对象,用于绘制图形.
...
def add_path_patch(ax, vertices, codes=None, facecolor='none', edgecolor=BLACK, auto_scale=True, lw=None, **kwargs):
    '''
    利用path创建patch.
...
def add_polygon_patch(ax, xy, facecolor='none', edgecolor=BLACK, auto_scale=True, lw=None, **kwargs):
    '''
    创建多边形patch.
...
def add_circle_patch(ax, center, radius, facecolor='none', edgecolor=BLACK, auto_scale=True, lw=None, **kwargs):
    '''
    创建圆形patch.
...
def add_arc_patch(ax, center, radius, theta1, theta2, facecolor='none', edgecolor=BLACK, auto_scale=True, lw=None, angle_type='rad', **kwargs):
    '''
    创建圆弧patch
...
def add_mid_arrow(ax, x_start, y_start, x_end, y_end, xycoords='data', label=None, fc=RED, ec=RED, linewidth=None, arrow_sytle=ARROW_STYLE, head_width=ARROW_HEAD_WIDTH, head_length=ARROW_HEAD_LENGTH, alpha=1.0, **kwargs):
    '''
...
def add_quiver_3d(ax, x_start, y_start, z_start, x_end, y_end, z_end, label=None, color=RED, linewidth=None, arrow_length_ratio=0.3, alpha=1.0, **kwargs):
    '''
...
def plt_edge_scatter(ax, x, y, color=BLUE, edge_color=BLACK, s=None, edge_prop=1.1, label=None, density=False, edge_kwargs=None, sc_kwargs=None):
    '''
    绘制带边界的散点图.
...
def plt_colorful_scatter(ax, x, y, c, cmap=CMAP, norm_mode='linear', vmin=None, vmax=None, norm_kwargs=None, s=None, label=None, label_cmap_float=1.0, scatter_kwargs=None, cbar=True, cbar_postion=None, cbar_kwargs=None):
    '''
    绘制颜色关于c值变化的散点图.
...
def plt_group_box(ax, x, y, label_list, width=None, inner_gap=0.0, colors=CMAP, vert=True, **kwargs):
    ...
    if width is None:
        ...
        # This is distributed as: num_bars * width + (num_bars - 1) * (width * inner_gap)
        # So: width = BAR_WIDTH / (num_bars + (num_bars - 1) * inner_gap)
        width = BAR_WIDTH / (num_boxes + (num_boxes - 1) * inner_gap)
    gap_width = width * inner_gap
...
def plt_colorful_scatter_3d(ax, x, y, z, c, cmap=CMAP, norm_mode='linear', vmin=None, vmax=None, norm_kwargs=None, s=None, label=None, label_cmap_float=1.0, scatter_kwargs=None, cbar=True, cbar_postion=None, cbar_kwargs=None):
    '''
    绘制颜色关于c值变化的散点图
...
def add_star_extreme_value(ax, x, y, extreme_type, label=None, marker=STAR, color=None, markersize=None, linestyle='None', **kwargs):
    '''
    在min或者max位置添加星号
...
def add_star_extreme_value_heatmap(ax, data, extreme_type, label=None, marker=STAR, color=None, markersize=None, linestyle='None', **kwargs):
    '''
    在热图的极值位置添加星号
    :param extreme_type: 极值类型,可以是'max'或者'min'
    '''
...
# region 复杂作图函数(添加辅助线)
def add_vline_extreme_value(ax, x, y, extreme_type, label=None, color=None, linestyle=AUXILIARY_LINE_STYLE, linewidth=None, **kwargs):
    '''
    在min或者max位置添加垂直线
    :param extreme_type: 极值类型,可以是'max'或者'min'
    '''
...
# region 通用函数(tick)
def add_nested_tick(ax, axis, ticks, labels, location=0, length=None, width=0):
    """
...
def add_sep_tick(ax, axis, ticks, length=None, width=None):
    """
...
def broken_axis(ax1, ax2, orientation, link_location='auto', share=True, offset=0.05, slope=1., color=BLACK, linewidth=None, **line_kwargs):
    '''
...
def get_fig_ax_by_fig_width_mm(nrows=1, ncols=1, ax_width=AX_WIDTH, ax_height=AX_HEIGHT, fig_width='single', sharex=False, sharey=False, rm_repeat_tick_label_when_share=RM_REPEAT_TICK_LABEL_WHEN_SHARE, subplots_params=None, squeeze=True, margin=None, label='ax'):
    ...
    '''
    if FONT_SIZE > 8:
        print("Warning: FONT_SIZE is greater than 8, please use 'set_font_size_paper_mode'")
    if LINE_WIDTH > 1.5:
        print("Warning: LINE_WIDTH is greater than 1.5, please use 'set_line_style_paper_mode'")
    subplots_params = update_dict({}, subplots_params)
    margin = update_dict(MARGIN, margin)
...
def get_fig_ax_by_fig_width_mm_3d(nrows=1, ncols=1, ax_width=AX_WIDTH, ax_height=AX_HEIGHT, fig_width='single', sharex=False, sharey=False, rm_repeat_tick_label_when_share=RM_REPEAT_TICK_LABEL_WHEN_SHARE, subplots_params=None, squeeze=True, margin=None, label='ax'):
    '''
    创建一个3D图形和轴对象,并以fig宽度(mm)作为主控制量自动推导fig高度.
    '''
    subplots_params = update_dict({}, subplots_params)
    if 'projection' not in subplots_params:
        subplots_params['projection'] = '3d'
    margin = update_dict(MARGIN_3D, margin)
    return get_fig_ax_by_fig_width_mm(
        nrows=nrows,
        ncols=ncols,
        ax_width=ax_width,
        ax_height=ax_height,
        ...
        sharex=sharex,
        sharey=sharey,
        rm_repeat_tick_label_when_share=rm_repeat_tick_label_when_share,
        subplots_params=subplots_params,
        squeeze=squeeze,
        margin=margin,
        label=label,
    )
